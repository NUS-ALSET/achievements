...
class CodeCombatActivity extends React.PureComponent {
  ...
  updateCodeCombatProfile = () => {
    ...
    dispatch(externalProfileRefreshRequest(codeCombatProfileId, "CodeCombat"));
  };
  render() {
    ...
    const updateCodeCombatAchievements = (
      <Button
        color="primary"
        onClick={this.updateCodeCombatProfile}
    ...
    if (problem.type === ACTIVITY_TYPES.codeCombat.id) {
      ...
      const totalAchievements =
        (userAchievements.CodeCombat || {}).totalAchievements || 0;
      ...
      return (
        <div>
          ...
          <Typography>Would you like to go to CodeCombat.</Typography>
          <br />
          <Button color="primary" variant="contained">
            <a
