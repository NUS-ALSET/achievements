...
let simulate = function(botFiles, config) {
  ...
  let tableStart2 =
    "<tr id='game-result-header'><td>Totals as Player 1 &#8595;</td>";
  let tableStart3 = "";
  ...
  for (let bot1 of botFiles) {
    ...
    for (let bot2 of botFiles) {
      ...
      if (bot1 !== bot2) {
        ...
        if (result.player1 > result.player2) {
          tableStartArr[botFiles.indexOf(bot1)] +=
            "<td><a class='restartGame' data-bot1='" +
            bot1.name +
            "' data-bot2='" +
            bot2.name +
            "' href='#'>1.0</a></td>";
    ...
    tableStartArr[botFiles.indexOf(bot1)] +=
      "<td>" + player1Score[botFiles.indexOf(bot1)].toFixed(1) + "</td>";
  }
  tableStart2 += "</tr>";
  for (var i = botFiles.length - 1; i >= 0; i--) {
    tableStart += "<td>" + player2Score[i].toFixed(1) + "</td>";
    tableStartArr[i] +=
      "<td>" + (player1Score[i] + player2Score[i]).toFixed(1) + "</td></tr>";
    tableStart3 += tableStartArr[i];
  }
  tableStart +=
    "<td rowspan=2>Totals as Player1</td><td rowspan=2>Overall Total</td></tr>";
