  ...
  if (player.path.length == 0 && !world.player.passenger) {
    if (random > 0.2)
      world.collectives.forEach(passenger => {
        if (closestPassenger === false) closestPassenger = passenger;
        else if (
          Math.sqrt(
          ...
          Math.sqrt(
            Math.pow(player.x - passenger.x, 2) +
              Math.pow(player.y - passenger.y, 2)
          )
        ) {
          closestPassenger = passenger;
        }
      });
    else closestPassenger = world.collectives[0];
    if (closestPassenger) {
      //finding path to the closest passenger
      var pointA = {
        x: Math.floor(player.x / world.config.roadWidth),
        y: Math.floor(player.y / world.config.roadWidth)
      };
      var pointB = {
        x: Math.floor(closestPassenger.x / world.config.roadWidth),
        y: Math.floor(closestPassenger.y / world.config.roadWidth)
      };
    ...
    var pointB = {
      x: Math.floor(player.passenger.takeofX / world.config.roadWidth),
      y: Math.floor(player.passenger.takeofY / world.config.roadWidth)
    };
    player.path = findShortestPath(world.map, pointA, pointB, world.index);
