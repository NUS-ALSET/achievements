  ...
  attachClickEvent() {
    ...
    for (var i = 0; i < restartGame.length; i++) {
      restartGame[i].onclick = e => {
        ...
        Store.player1ControlSelected =
          e.target.attributes[1].value == "level1" || "level2" || "level3"
            ? e.target.attributes[1].value
            : "custom code";
        Store.player2ControlSelected =
          e.target.attributes[2].value == "level1" || "level2" || "level3"
          ...
          <button
            ...
            onClick={() => {
              ...
              var result = tableResult(
                [Store.func, level1, level2, level3],
                config
              );
              this.setState({ presult: result, showTable: true });
            }}
