...
import {
  defaultJavascriptFunctionCode,
  defaultPythonCodeFunction
} from "./Components/defaultCode";
...
class Updater extends Component {
  ...
  loop = () => {
    if (this.props.store.mode == PLAY) {
      const gameOver =
        this.props.store.time <= 0
          ? {
              status: true,
              winner: null,
              message: "Time Over"
            }
          : this.props.store.score[0] >= this.props.store.scoreToWin ||
            this.props.store.score[1] >= this.props.store.scoreToWin
          ? {
              status: true,
              winner:
                this.props.store.score[0] === this.props.store.score[1]
                  ? 0
                  : this.props.store.score[1] >= this.props.store.scoreToWin
                  ? 2
                  : 1,
  ...
  updateStateFromProps(props) {
    if (props.player1Data) {
      ...
      this.props.store.scoreToWin =
        props.gameData.scoreToWin || config.scoreToWin;
      this.props.store.botsQuantity = Math.min(
        props.gameData.botsQuantities || props.store.botsQuantity,
        config.maxBotsQuantityPerGame
      );
      this.props.store.currentLevel = Math.min(
        Number(props.gameData.levelsToWin) || 1,
        3
      );
      if (this.props.playAsPlayer2) {
        this.props.store.player2Func =
          props.gameData.playMode === CUSTOM_CODE
            ? props.player1Data.jsCode || defaultJavascriptFunctionCode
            : control;
        this.props.store.player1Func =
          (props.player2Data || {}).jsCode ||
          levels[props.gameData.levelsToWin - 1];
      } else {
        this.props.store.player1Func =
          props.gameData.playMode === CUSTOM_CODE
            ? props.player1Data.jsCode || defaultJavascriptFunctionCode
  ...
  submitSolition = () => {
    this.props.onCommit({
      status:
        this.state.gameOver.winner === 0
          ? "DRAW"
          : this.state.gameOver.winner == 1
          ? "WON"
          : "LOST",
      result:
        this.state.gameOver.winner === 0
      ...
      jsCode:
        this.props.gameData.playMode === CUSTOM_CODE &&
        this.props.store.editorMode === "javascript"
          ? this.props.store.player1Func.toString()
          : "",
      pyCode:
        this.props.gameData.playMode === CUSTOM_CODE &&
        this.props.store.editorMode === "python"
          ? this.props.store.editorPyCode
          : ""
    });
  };
  componentDidMount() {
    ...
    return (
      <div>
        <WinningScreen
          ...
          submitSolition={this.submitSolition}
        />
        <ScoreDisplay
