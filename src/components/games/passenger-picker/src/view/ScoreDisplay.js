...
const ScoreDisplay = ({
    ...
    return <div>
        ...
        <div className="score-display">
            <div> Player 1: {playAsPlayer2 ? `${levels[Number(store.currentLevel) - 1]}-Bot` : playersName ? playersName['player1'] : 'Me'}</div>
            <div> Player 2: {!playAsPlayer2 ? `${levels[Number(store.currentLevel) - 1]}-Bot` : playersName ? playersName['player1'] : 'Me'}</div>
        </div>
