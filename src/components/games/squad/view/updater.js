...
class Updater extends Component {
    ...
    if (Store.mode == "play") {
      this.gameOver =
        Store.time <= 0
          ...
          : Store.score[0] >= Store.scoreToWin ||
            Store.score[1] >= Store.scoreToWin
          ? {
              status: true,
              winner:
  ...
  pauseResumeGame() {
    if (Store.mode == "play") Store.mode = "pause";
    else Store.mode = "play";
  ...
  submitSolition = () => {
    this.props.onCommit({
      ...
      jsCode:
        this.props.player1Data.playMode === "custom code"
          ? Store.player1Func.toString()
          : ""
    });
  };
  render() {
      ...
      <div>
        ...
        >
          ...
          <div>
            ...
            <p style={{ margin: 0, textAlign: "center" }}>
              Score to win:{Store.scoreToWin}
            </p>
            <button onClick={() => this.restartGame()}>Restart</button>
            <button onClick={() => this.pauseResumeGame()}>
