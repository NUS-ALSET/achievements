...
function Simulation(config, bot1clb, bot2clb) {
  ...
  if (
    !this.config.collectiveSize ||
    typeof this.config.collectiveSize != "number"
  ) {
    ...
    for (var gameId = 0; gameId < gamesQuant; gameId++) {
      if (e.key == this.config["player" + (gameId + 1) + "Keys"].up) {
        ...
        this.controlInfo.current[gameId] =
          this.controlInfo.current[gameId] == 0 ? 1 : 0;
      }
    }
  });
}
...
Simulation.prototype.simulate = function() {
  var bot1_1Data = {
    player: this.bots[0][0],
    collectives: this.collectives[0],
  ...
  var bot2_1Data = {
    player: this.bots[1][0],
    collectives: this.collectives[1],
    direction: this.direction[1][0],
    index: 0,
    config: this.config,
    gameId: 1,
    controlInfo: this.controlInfo
...
Simulation.prototype.generateCollectives = function() {
  if (this.collectives[0].length > 0 && this.collectives[1].length > 0) return;
  ...
  for (var i = 0; i < stonesQuant; i++) {
    ...
    stoneObj.x =
      Math.floor(Math.random() * (gameWidth / playerSize - -0) + 0) *
      playerSize;
    stoneObj.y =
      Math.floor(Math.random() * (gameHeight / playerSize - 0) + 0) *
      playerSize;
    stoneObj.size = size;
...
Simulation.prototype.moveCharacters = function() {
  ...
  if (
    this.bots[0][0].x == this.bots[0][1].x &&
    this.bots[0][0].y == this.bots[0][1].y
  ) {
  ...
  if (
    this.bots[1][0].x == this.bots[1][1].x &&
    this.bots[1][0].y == this.bots[1][1].y
  ) {
