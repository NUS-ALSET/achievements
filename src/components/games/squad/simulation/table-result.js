...
let simulate = function(botFiles, config) {
  ...
  for (let botFile1 of botFiles) {
    ...
    for (let botFile2 of botFiles) {
      ...
      if (tableCol[botFile1.name + "_" + botFile2.name]) {
        tableCol[botFile1.name + "_" + botFile2.name][player1Index - 1].push(
          res1.toFixed(1)
        );
        tableCol[botFile1.name + "_" + botFile2.name][player2Index - 1].push(
          res2.toFixed(1)
        );
      } else if (tableCol[botFile2.name + "_" + botFile1.name]) {
        tableCol[botFile2.name + "_" + botFile1.name][player1Index - 1].push(
          res1.toFixed(1)
        );
        tableCol[botFile2.name + "_" + botFile1.name][player2Index - 1].push(
          res2.toFixed(1)
        );
      } else {
        tableCol[botFile1.name + "_" + botFile2.name] = new Array(
          botFiles.length
        );
        for (
          var i = 0;
          i < tableCol[botFile1.name + "_" + botFile2.name].length;
          i++
        ) {
      ...
      if (!sum[player1Index - 1]) sum[player1Index - 1] = 0;
      sum[player1Index - 1] += res1;
      if (!sum[player2Index - 1]) sum[player2Index - 1] = 0;
  ...
  botFiles.forEach((element, index) => {
    tableStart +=
      "<tr><td>" + (index + 1) + "</td><td>" + element.name + "</td>";
    ...
    tableStart +=
      "<td align='center'>" + (order.indexOf(sum[index]) + 1) + "</td>";
    tableStart += "</tr>";
  });
  tableStart += "</table>";
  return tableStart;
